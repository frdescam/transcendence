		...
		<div class="fit column q-pa-sm" style="background: rgba(0, 0, 0, 0.4); border-radius: 10px">
			<div class="q-pa-sm row col-3 items-center">
				...
				<div v-bind:class="$q.screen.lt.md ? 'text-center' : 'text-right'" class="col-12 col-md-6"
					style="font-size: 1.5em; color: #eee;">{{ $t('index.ratio') }}: {{ user.ratio }}%</div>
			</div>
			<q-badge
			...
			<div class="row justify-between col-2 items-end">
				<div style="color: #eee;" class="q-mb-none">{{ capitalize($t('index.nextLevel')) }}</div>
				<div style="color: #eee;" class="q-mb-none">{{ parseInt((user.xp - parseInt(user.xp)) * 100) }} / 100 {{ $t('index.exp').toUpperCase() }}</div>
			</div>
			<div class="col-2">
